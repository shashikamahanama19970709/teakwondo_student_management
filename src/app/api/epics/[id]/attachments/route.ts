...
export async function GET(
  ...
  try {
    ...
    await connectDB()
    const epic = await Epic.findById(params.id)
    
    if (!epic) {
      return NextResponse.json({ error: 'Epic not found' }, { status: 404 })
    }

    return NextResponse.json(epic.attachments)
  } catch (error) {
...
export async function POST(
  ...
  try {
    ...
    epic.attachments.push(attachment)
    await epic.save()
...
export async function DELETE(
  ...
  try {
    ...
    if (index < 0 || index >= epic.attachments.length) {
      return NextResponse.json({ error: 'Invalid attachment index' }, { status: 400 })
    }

    epic.attachments.splice(index, 1)
    await epic.save()
